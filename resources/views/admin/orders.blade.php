        ...
        <table class="aorders-data-table">
            ...
            <tbody id="order-table-body">
                @foreach ($orders as $order)
                    ...
                    @endphp
                    <tr data-order-id="{{ $order->order_code }}">
                        ...
                        <td>
                            @if (in_array($order->status, ['Chờ xác nhận', 'Đã xác nhận', 'Đang giao hàng']))
                                <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="progress" value="1">
                                    <button type="submit" class="aorders-btn aorders-btn-primary"><i class="fas fa-check"></i></button>
                                </form>
                            @endif
                        </td>
                    </tr>
