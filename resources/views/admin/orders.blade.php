            ...
            <table class="aorders-data-table">
                ...
                <tbody id="order-table-body">
                    @foreach ($orders as $order)
                        ...
                        @endphp
                        <tr data-order-id="#DH-{{ $order->order_code }}">
                            ...
                            <td>
                                ...
                                @if ($nextStatus)
                                    <form action="{{ route('admin.orders.update', $order->id) }}" method="POST" style="display: inline;">
                                        ...
                                        <button type="submit" class="aorders-btn aorders-btn-primary">✓</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function filterRows() {
                ...
                rows = Array.from(tableBody.querySelectorAll("tr")).filter((row) => {
                    ...
                    return orderCode.includes(searchTerm) || customer.includes(searchTerm) || phone.includes(searchTerm) || status.includes(searchTerm);
                });
                currentPage = 1;
                renderPage();
                renderPagination();
