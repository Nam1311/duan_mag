                ...
                <tbody id="order-table-body">
                    @foreach ($orders as $order)
                        @php
                            ...
                            // Lấy tên khách hàng
                            $customerName = !empty($order->address_details['receiver_name'])
                                ? $order->address_details['receiver_name']
                                : 'Không xác định';
                            // Lấy số điện thoại
                            $phone = !empty($order->address_details['phone'])
                            ...
                            <td>
                                ...
                                @else
                                    <form action="{{ route('admin.orders.update', $order->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('PUT')
                                        <select name="status" class="aorders-status-select status-{{ $color }}"
                                            onchange="this.form.submit()">
                                            <option value="Chờ xác nhận"
                                                {{ $order->status == 'Chờ xác nhận' ? 'selected' : '' }}>Chờ xác nhận
                                            </option>
                                            <option value="Đã xác nhận"
                                                {{ $order->status == 'Đã xác nhận' ? 'selected' : '' }}>Đã xác nhận
                                            </option>
                                            <option value="Đang giao hàng"
                                                {{ $order->status == 'Đang giao hàng' ? 'selected' : '' }}>Đang giao hàng
                                            </option>
                                            <option value="Thành công"
                                                {{ $order->status == 'Thành công' ? 'selected' : '' }}>Thành công</option>
                                            <option value="Đã hủy" {{ $order->status == 'Đã hủy' ? 'selected' : '' }}>Đã
                                                hủy</option>
                                            <option value="Hoàn hàng"
                                                {{ $order->status == 'Hoàn hàng' ? 'selected' : '' }}>Hoàn hàng</option>
                                        </select>
                                    </form>
                                @endif
                            </td>
                            <td>
                                <form action="{{ route('admin.orders.softDelete', $order->id) }}" method="POST"
                                    style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="aorders-btn aorders-btn-delete"
                                        {{ $order->status != 'Đã hủy' ? 'disabled' : '' }}>
                                        Xóa
                                    </button>
                                </form>
                                <a href="{{ route('admin.orders.show', $order->id) }}"
                                    class="aorders-btn aorders-btn-primary">Xem</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function filterRows() {
                ...
                rows = Array.from(tableBody.querySelectorAll("tr")).filter((row) => {
                    ...
                    return customer.includes(searchTerm) || phone.includes(searchTerm) || status.includes(
                        searchTerm);
                });
                currentPage = 1;
                renderPage();
