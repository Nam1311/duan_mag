...
<body>
    <div class="container py-4">
        ...
        <div class="card order-card">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0" style="flex: 1;">Thông tin đơn hàng</h5>
                     <span style="margin-right: 10px;" class="status-badge bg-primary text-white">
                        {{ $order->status_payment == "Đã thanh toán" ? "Đã thanh toán" : "Chưa thanh toán" }}
                    </span>
            ...
            <div class="card-body">
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <p class="mb-0 text-muted">{{ $order->address_text }}</p>
                    </div>
                    <div class="col-md-6">
                        <h6>Thông tin đơn hàng</h6>
                        <p class="mb-1">Ngày đặt: {{ $order->created_at->format('d/m/Y H:i') }}</p>
                        <p class="mb-1">Phương thức thanh toán: {{ $order->payment_methods ?? 'N/A' }}</p>
                        @if($order->voucher)
                            <p class="mb-1">Mã giảm giá: {{ $order->voucher->code }}
                        ...
                        <tbody>
                            @foreach($order->orderDetails as $item)
                                @php
                                    ...
                                    // Lấy ảnh chính của sản phẩm (thumbnail)
                                    $productImg = $product->thumbnail ? $product->thumbnail->path : ($product->images->first() ? $product->images->first()->path : null);

                                    // Sử dụng giá từ productVariant
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            @if($productImg)
                                                <img src="{{ asset( $productImg) }}" alt="{{ $product->name }}"
                                            ...
                                            <div>
                                                ...
                                                <p class="mb-0 text-muted small">Mã SP: {{ $order->order_code }}</p>
                                                @if($item->productVariant->size)
                                                    <p class="mb-0 text-muted small">Size: {{ $item->productVariant->size->name }}</p>
                                                @endif
                                                @if($item->productVariant->color)
                                                    <p class="mb-0 text-muted small">Màu: {{ $item->productVariant->color->name }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="summary-card">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        <span>Giảm giá:</span>
                        <span class="text-danger">-{{ number_format($voucherDiscount, 0, ',', '.') }}₫</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between fw-bold fs-5">
                        ...
                        <span class="text-primary">{{ number_format($order->orderDetails->sum('total_final'), 0, ',', '.') }}₫</span>
                    </div>
        ...
        <footer class="mt-5 text-center text-muted">
            <p>Nếu bạn cần hỗ trợ, vui lòng liên hệ <a href="mailto:support@mag.com">support@mag.com</a> hoặc 0962615032
            </p>
